...
const SongTable = (props: ISongcomponent) => {
  ...
  const removeSong = async () => {
    const url = `${EndPoints.removeSongUrl}/${id}/songs`;
    ...
    },
    {
      ...
      render: (item: String, record: any) => (
        <div style={{ display: "flex", gap: 10 }}>
          <Button
            type="primary"
            onClick={() => navigate(`/songs/${record._id}`)}
          >
            Detail
          </Button>
          {id && (
          ...
          {!id && (
            <Button
              icon={<EditOutlined />}
              onClick={() => navigate(`/songs/update/${record._id}`)}
            >
              Update
            </Button>
          )}
  ...
  return (
    <div style={{ width: "60%" }}>
      ...
      {!id && (
        <Button type="link">
          <Link to="/songs/create">Create New</Link>
        </Button>
      )}
      <Table rowKey={(record) => record._id!} columns={columns} dataSource={songs} rowClassName="table-row" />
    </div>
  );
};
